...
#对比损失函数
def contr_loss(memory_bank, pred_rep, labels, mask, which_memory, num_classes, memory_bank_index=0, temp=0.5,mask_memory=None):
...
def contr_self_loss(memory_bank, pred_rep, labels, mask,num_classes, temp=0.5):
    b, num_feat, w, j = pred_rep.shape
...
def main():
    ...
    model_l.load_state_dict(torch.load('D:\\xh\\ClassMix-master\\saved\\DeepLabv2\\label 2975\\checkpoint-iter80000.pth')['model'])
    model_r.load_state_dict(
        torch.load('D:\\xh\\ClassMix-master\\saved\\DeepLabv2\\cross teacher 80000 63.7\\checkpoint-iter80000.pth')['model'])
    ...
    new_data_loader=get_loader('new_city')
    new_data_path=get_data_path('new_city')
    if random_crop:
        data_aug = RandomCrop_city(input_size)
    else:
        data_aug = None

    trainloader_remain_loader = new_data_loader(new_data_path, is_transform=True, augmentations=data_aug)
    trainloader_remain = data.DataLoader(trainloader_remain_loader,batch_size=batch_size, num_workers=1,pin_memory=True)
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        if lr_schedule:
            adjust_learning_rate(optimizer_l, i_iter+150000)
            adjust_learning_rate(optimizer_r, i_iter+150000)
        ...
        if train_unlabeled:
            ...
            # ema损失
            L_u_ema_l = ema_loss(model_l, ema_model_r, images_remain, inputs_u_w, weak_parameters, interp, i_iter,unlabeled_loss)
            L_u_ema_r = ema_loss(model_r, ema_model_l, images_remain, inputs_u_w, weak_parameters, interp, i_iter,unlabeled_loss)
        ...
        # 这个写在ema模型更新前面
            ...
            #     print()
            #     print(memory_bank_size_l)
            #     print(memory_bank_size_r)
