...
from deeplabv3plus.network import SingleNetwork
...
pretrained_model='/home/ty/mycode/resnet101.pth'
...
def create_ema_model(model):
    ema_model = SingleNetwork(num_classes=num_classes,norm_layer=SynchronizedBatchNorm2d)
...
def main():
    ...
    # create network
    model_l = SingleNetwork(num_classes, norm_layer=SynchronizedBatchNorm2d, pretrained_model=pretrained_model)
    model_r = SingleNetwork(num_classes, norm_layer=SynchronizedBatchNorm2d, pretrained_model=pretrained_model)

    init_weight(model_l.business_layer, nn.init.kaiming_normal_,
                SynchronizedBatchNorm2d, 1e-5, momentum,
    ...
    params_list_l = []
    params_list_l = group_weight(params_list_l, model_l.backbone,
                                 SynchronizedBatchNorm2d, base_lr)
    for module in model_l.business_layer:
        params_list_l = group_weight(params_list_l, module, SynchronizedBatchNorm2d,
                                     base_lr)  # head lr * 10

    params_list_r = []
    params_list_r = group_weight(params_list_r, model_r.backbone,
                                 SynchronizedBatchNorm2d, base_lr)
    for module in model_r.business_layer:
        params_list_r = group_weight(params_list_r, module, SynchronizedBatchNorm2d,
                                     base_lr)  # head lr * 10
    ...
    labeled_data_loader=get_loader("pascal_off_voc_labeled")
    data_path = get_data_path(dataset)
    train_labeled_dataset = labeled_data_loader(data_path, crop_size=input_size, scale=random_scale, mirror=random_flip)
    sample_labeled_size = len(train_labeled_dataset)
    sampler_labeled = torch.utils.data.sampler.SubsetRandomSampler(
        np.random.choice(range(len(train_labeled_dataset)), sample_labeled_size))
    trainloader = data.DataLoader(train_labeled_dataset,
                                  batch_size=batch_size, sampler=sampler_labeled, num_workers=num_workers,
                                  pin_memory=True)
    trainloader_iter = iter(trainloader)

    unlabeled_data_loader = get_loader("pascal_off_voc_unlabeled")
    data_path = get_data_path(dataset)
    train_unlabeled_dataset = unlabeled_data_loader(data_path, crop_size=input_size, scale=random_scale, mirror=random_flip)
    sample_unlabeled_size = len(train_unlabeled_dataset)
    sampler_unlabeled = torch.utils.data.sampler.SubsetRandomSampler(
        np.random.choice(range(len(train_unlabeled_dataset)), sample_unlabeled_size))
    trainloader_remain = data.DataLoader(train_unlabeled_dataset,
                                          batch_size=batch_size, sampler=sampler_unlabeled, num_workers=num_workers,
                                          pin_memory=True)
        ...
        if train_unlabeled:
            ...
            if with_embed and contr_train==True:
                ...
                L_l_contr_cross = contr_net_loss(torch.cat([memory_bank_l, memory_bank_r], dim=1), pred_rep_l[batch_size:,:,:],pred_rep_r[batch_size:,:,:].detach(), max_u_r, mask_l[batch_size:,:,:].int()==0,
                                                 (which_memory==0) & (prob_u_r>=gamma), num_classes)
                L_r_contr_cross = contr_net_loss(torch.cat([memory_bank_l, memory_bank_r], dim=1), pred_rep_r[batch_size:,:,:],pred_rep_l[batch_size:,:,:].detach(), max_u_l, mask_r[batch_size:,:,:].int()==0,
                                                 (which_memory==1) & (prob_u_l>=gamma), num_classes)
